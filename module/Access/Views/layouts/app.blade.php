	...
	<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled toolbar-fixed aside-enabled aside-fixed" style="--kt-toolbar-height:55px;--kt-toolbar-height-tablet-and-mobile:55px">
		...
		<div class="d-flex flex-column flex-root">
			...
			<div class="page d-flex flex-row flex-column-fluid">
				<!--begin::Aside-->
				@include('Access::layouts.partials.sidebar')
				<!--end::Aside-->

				<!--begin::Wrapper-->
				<div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
					<!--begin::Header-->
					@include('Access::layouts.partials.header')
					<!--end::Header-->
					...
					<!--begin::Footer-->
					@include('Access::layouts.partials.footer')
					<!--end::Footer-->
				</div>
				<!--end::Wrapper-->
			</div>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function () {
				// Check if there's a success message and open the modal
				var successMessage = document.querySelector('.alert-success');
				if (successMessage) {
					$('#successModal').modal('show'); // Assuming you are using Bootstrap modal
				}
			});
		</script>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function () {
				...
				const storageKey = 'asideMinimizeState';
				...
				toggleBtn.addEventListener('click', function () {
					setTimeout(() => {
						const isMinimized = body.hasAttribute('data-kt-aside-minimize');
						if (isMinimized) {
							localStorage.setItem(storageKey, 'on');
						} else {
							localStorage.setItem(storageKey, 'off');
						}
					}, 100); // Wait for Metronic toggle behavior to finish
				});
			});
		</script>
