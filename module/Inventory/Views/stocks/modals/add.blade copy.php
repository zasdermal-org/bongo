                    ...
                    <div id="stockContainer">
                        <template id="pair-template">
                            <div class="row mb-2 pair-group">
                                <div class="col-md-4">
                                    <select onchange="categoryName(this)" name="category_name" data-control="select2" data-placeholder="Select product category" class="form-select form-select-solid category-name">
                                        <option></option>
                                ...
                                <div class="col-md-6">
                                    <select name="product_id[]" data-control="select2" data-placeholder="Select products" class="form-select form-select-solid product-id">
                                        
                                    </select>
                                </div>

                                {{-- <div class="col-md-6">
                                    <select name="product_id[]" data-control="select2" data-placeholder="Select products" class="form-select form-select-solid">
                                        <option></option>
...
<script>
    function categoryName(element) {
        let $row = $(element).closest('.pair-group');
        let selectedCategory = $(element).find('option:selected').val();
        let action = "{{ route('catalog.products_by_category_name') }}" + '?category_name=' + encodeURIComponent(selectedCategory);

        let $productSelect = $row.find('.product-id');
        $productSelect.empty();
    ...
    function addPair() {
        ...
        $('#stockContainer .pair-group:last-child').find('select[data-control="select2"]').select2({
            dropdownParent: $('#kt_modal_add_permission')
        });
    }
