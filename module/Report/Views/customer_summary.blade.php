            ...
            <div class="card card-flush">
                ...
                <div class="card-header mt-5">
                    ...
                    <div class="card-title">
                        <form class="d-flex align-items-center position-relative my-1" action="{{ route('report.customer_summary') }}" method="GET">
                            <div class="w-110 mw-120px me-2">
                                <select name="sale_point_id" class="form-select form-select-solid" data-control="select2">
                                    <option value="">All Sale Point</option>
                ...
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                        ...
                        <thead>
                            ...
                            <tr class="text-start text-black-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-120px">Sales Point Name</th>
                                <th class="min-w-80px">Opening Blance</th>
                                <th class="min-w-80px">Sales</th>
                                <th class="min-w-80px">Commission</th>
                                <th class="min-w-80px">Return</th>
                                <th class="min-w-80px">Collection</th>
                                <th class="min-w-80px text-end">Closing Due</th>
                            </tr>
                            <!--end::Table row-->
                        </thead>
                        <!--end::Table head-->
                        ...
                        <tbody class="text-black-600">
                            @foreach ($salesPoint as $key => $salePoint)
                                <tr class="hover-row">
                                    <td>{{ $salesPoint->firstItem() + $key }}</td>

                                    <td>{{ $salePoint->name }} ({{ $salePoint->code_number }})</td>
                                    
                                    <td>{{ number_format($salePoint->opening_balance, 2) }}</td>

                                    <td>{{ number_format($salePoint->sales, 2) }}</td>

                                    <td>{{ number_format($salePoint->commission, 2) }}</td>

                                    <td>{{ number_format($salePoint->return, 2) }}</td>

                                    <td>{{ number_format($salePoint->collection, 2) }}</td>

                                    <td>{{ number_format($salePoint->closing_due, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <!--end::Table body-->
                    ...
                    <div class="pagination-container mt-10">
                        <nav>
                            {{ $salesPoint->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-5') }}
                        </nav>
                    </div>
                </div>
                <!--end::Card body-->
